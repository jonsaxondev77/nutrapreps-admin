...
export default function StatisticsChart() {
  const { data: chartData, isLoading, error } = useGetStatisticsChartDataQuery();

  const labels = chartData?.map(d => d.label) || [];
  const salesData = chartData?.map(d => d.salesCount) || [];
  const revenueData = chartData?.map(d => parseFloat(d.revenue.toFixed(2))) || [];
      ...
      <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
        <div className="w-full">
          ...
          <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
            Sales and Revenue for the last 12 months
          </p>
        </div>
